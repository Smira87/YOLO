...
def plot_bboxes(image, boxes, labels=[], colors=[], score=True, conf=None):
    ...
    global picked_box, picked_target_x, picked_target_yaw
    picked_target_x = picked_target_y = 0
    picked_box = None
    # plot each boxes
    for box in boxes:
    ...
    cv2.rectangle(image, (picked_target_x, picked_target_y), (picked_target_x + 4, picked_target_y + 4),
    ...
    print('============')
    if picked_box is not None:
        if conf:
            if picked_box[-2] > conf:
                color = colors[int(picked_box[-1])]
                box_label(image, picked_box, label, color)
        else:
            color = colors[int(picked_box[-1])]
            box_label(image, picked_box, label, color)
